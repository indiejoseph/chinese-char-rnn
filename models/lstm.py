...
class BNLSTMCell(RNNCell):
    ...
    def __init__(self, num_units, forget_bias=1.0, activation=tanh, reuse=None, training=False):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__, reuse=self._reuse):
          ...
          bn_xh = batch_norm(xh, 'xh', self._training)
          bn_hh = batch_norm(hh, 'hh', self._training)
          ...
          new_c = (c * sigmoid(f + self._forget_bias) + sigmoid(i) *
                   self._activation(j))
          bn_new_c = batch_norm(new_c, 'c', self._training)
